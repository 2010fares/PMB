...
// +-------------------------------------------------+
// $Id: notices.inc.php,v 1.15.2.1 2013-11-13 10:56:31 dgoron Exp $
	...
	case "resa_a_traiter" :
		switch($dest) {
			case "TABLEAU":
				...
				$worksheet->write(2,2,$msg[233]);
				$worksheet->write(2,3,$msg[298]);
				$worksheet->write(2,4,$msg[295]);
				$worksheet->write(2,5,$msg[296]);
				$worksheet->write(2,6,$msg[297]);
				$worksheet->write(2,7,$msg[294]);
				$worksheet->write(2,8,$msg[232]);
					...
					if (count($tableau_expl_dispo)) {
						for ($i=0;$i<count($tableau_expl_dispo);$i++) {
							if (!$f_dispo_loc || ($tableau_expl_dispo[$i]['idlocation'] == $f_dispo_loc)) {
								...
								$worksheet->write(($line+3),2,$tableau_resa[$j]['resa_tit']);
								$worksheet->write(($line+3),3,$tableau_expl_dispo[$i]['location']);
								$worksheet->write(($line+3),4,$tableau_expl_dispo[$i]['section']);
								$worksheet->write(($line+3),5,$tableau_expl_dispo[$i]['expl_cote']);
								$worksheet->write(($line+3),6,$tableau_expl_dispo[$i]['statut']);
								$worksheet->write(($line+3),7,$tableau_expl_dispo[$i]['support']);
								$worksheet->write(($line+3),8,$tableau_expl_dispo[$i]['expl_cb']);
								$line++;
							}
						}
					}
