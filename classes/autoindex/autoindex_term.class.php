...
// +-------------------------------------------------+
// $Id: autoindex_term.class.php,v 1.1.2.6 2013-10-31 13:55:01 dbellamy Exp $
...
class autoindex_term {
	 ...
	 * @access public
	 */
	public function calc_term_document_distance( $clean_text='', $lang='fr_FR') {
		global $autoindex_distance_ratio;
		
		if(!$this->id || $clean_text==='') return;
		...
		$dmax = str_word_count($clean_text, 0, "0123456789");
		$this->document_distance = $dmax;
		$dterm = $dmax;
		if (preg_match_all($expr, ' '.$clean_text.' ',$matches, PREG_SET_ORDER)) {
...
//		$dmax = strlen($clean_text);
// 		$this->document_distance = $dmax;
// 		$dterm = 0;
// 		$expr = str_replace(' ', "(.*?)", "/(.*?)".$clean_label."/");	
// 		if (preg_match($expr, $clean_text,$matches )) {
